...
jest.mock("@/utils/contract-changer-api/contract-changer-api-client", () => ({
  contractChangerApiClient: {
    removeAddressFromWhitelist: jest.fn(() => ({ tx_hash: "mock_tx_hash" })),
  },
}))
...
describe("No silo", () => {
  ...
  it("[DELETE] returns 404 if silo not found", async () => {
...
describe("Silo whitelist permissions", () => {
  ...
  describe("Delete an address from the whitelist", () => {
    beforeEach(() => {
      mockSupabaseClient
        .from("silos")
    ...
    it("creates new tx if it doesn't exist", async () => {
      ...
      expect(
        contractChangerApiClient.removeAddressFromWhitelist,
