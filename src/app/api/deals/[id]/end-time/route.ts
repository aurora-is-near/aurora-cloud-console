...
export const GET = createApiEndpoint(
  ...
  async (_req: NextRequest, ctx: ApiRequestContext) => {
    ...
    return {
      endTime: result.data.end_time ? new Date(result.data.end_time) : null,
    }
  },
)
...
export const POST = createApiEndpoint(
  ...
  async (req: NextRequest, ctx: ApiRequestContext) => {
    ...
    const unixEndTime = Date.parse(endTime)

    if (isNaN(unixEndTime)) {
      abort(400, `Not a valid date: ${endTime}`)
    }
    ...
    const varKey = getDealVarKey(ctx.team.id, Number(ctx.params.id), "endTime")
    ...
    await proxyApiClient.update([
      ...
      },
      {
        ...
        var_type: "number",
        var_key: varKey,
        number_value: unixEndTime,
      },
    ])

    return { endTime: new Date(unixEndTime) }
  },
)
    ...
    await proxyApiClient.update([
      {
        op_type: "unset",
        var_type: "number",
        var_key: getDealVarKey(ctx.team.id, Number(ctx.params.id), "endTime"),
      },
    ])
